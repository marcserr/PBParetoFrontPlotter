...
class ParetoPlot:
    """
    Given a solved PB it plots its Pareto Front and our fair solution
    """
    ...
    def __init__(self, solutions, proposals_data, budget, city_bud, citizens, part, real_solution, theta):
        ...
        Parameters:
            ...
            theta (float): The minimum contribution percentage parameter theta for the Nash stakeholder solutions
        """
    ...
    def plot_pareto_and_sols(self, savefile):
        """
        Plots the whole Pareto front, our fair solution, and the ideal point.
        ...
        ax.scatter([1], [1], [1], c="black",s=60, marker="D", label="Ideal")
        #If there are points in the Pareto front that have not been plotted

        # We find the axis positions for our fair solution (that is their percentage of the maximum
        ...
        if self.real_solution:
            ...
            ax.scatter(real_sol_utils[0], real_sol_utils[1], real_sol_utils[2], c="orange",s=110, marker="X", label="Real solution", zorder = 2)

        # We print the results for our solution
        ...
        # In order to plot the solution with a different style of point, we remove it from the
        # rest of the Pareto front.
        sat_points.pop(fair_pos)
        alg_points.pop(fair_pos)
        npar_points.pop(fair_pos)

        # If there are points in the Pareto front after having removed our fair solution we plot them
        #We have to plot them one by one in order to impose a zorder so that they don't hide the solution
        for i in range(len(sat_points)):
            if i == 0:
                ax.scatter(sat_points[i], alg_points[i], npar_points[i], c="blue", label="Pareto front", zorder = 1)
            else:
                ax.scatter(sat_points[i], alg_points[i], npar_points[i], c="blue", zorder=1)

        # We plot our fair solution in a different color
        ax.scatter(fair_sol[0], fair_sol[1], fair_sol[2], c="red",s=220, marker="*", label="Fair solution", zorder=4)
        ...
        self.two_stakeholder_plot(sat_points, alg_points, npar_points, fair_sol, savefile, real_sol_utils)
    ...
    def three_nash_sol(self):
        """
        Find the 3-stakeholder coalition Nash solution of those in the Pareto front.

        Returns:
            nas_sol_pos (int): The solution id of the Nash solution for the 3-stakeholder coalition
        """
    ...
    def two_nash_sol(self, s1, s2):
        """
        Find a 2-stakeholder coalition Nash solution of those in the Pareto front.

        Parameters:
            s1 (string): The name of the first stakeholder in the coalition (has to be pc, gov, or np)
            s2 (string): The name of the second stakeholder in the coalition (has to be pc, gov, or np and different to s1)

        Returns:
            nash_pos (int): The solution id of the Nash solution for the 2-stakeholder coalition.
        """
        ...
        # Solution for participants and government
        if s1 in ["pc", "gov"] and s2 in ["pc", "gov"] and s1 != s2 and cpc + cgov > 0.5:
            for i in range(len(self.sat_points)):
        ...
        # Solution for participants and non-participants
        if s1 in ["pc", "np"] and s2 in ["pc", "np"] and s1 != s2 and cpc + cnp > 0.5:
            for i in range(len(self.sat_points)):
        ...
        # Solution for government and non-participants
        if s1 in ["gov", "np"] and s2 in ["gov", "np"] and s1 != s2 and cgov + cnp > 0.5:
    ...
    def fair_sol(self):
        ...
        # Find the Nash solution for the 3-stakeholder coalition
        nash_sol_id = self.three_nash_sol()

        # Find the Nash solution for the coalition of participants and government
        nash_pos_par_gov = self.two_nash_sol("pc", "gov")

        # Find the Nash solution for the coalition of participants and non-participants
        nash_pos_par_npar = self.two_nash_sol("pc", "np")

        # Find the Nash solution for the coalition of government and non-participants
        nash_pos_gov_npar = self.two_nash_sol("gov", "np")
        ...
        for i in range(len(self.sat_points)):
            ...
            # Complaint with regard to the participant and government coalition Nash solution
            if nash_pos_par_gov:
            ...
            # Complaint with regard to the participant and non-participant coalition Nash solution
            ...
            # Disparity with regard to the government and non-participant coalition Nash solution
    ...
    def two_stakeholder_plot(self, satpoints, algpoints, nparpoints, fairsol, savefile, realsol = None):
        """
        ...
        self.par_gov_plot(satpoints, algpoints, fairsol, newsavefile, realsol)
        newsavefile = savefile.replace(".pdf", "_parnpar.pdf")
        self.par_npar_plot(satpoints, nparpoints, fairsol, newsavefile, realsol)
        newsavefile = savefile.replace(".pdf", "_govnpar.pdf")
        self.gov_npar_plot(algpoints, nparpoints, fairsol, newsavefile, realsol)

    def par_gov_plot(self, satpoints, algpoints, fair_sol, savefile, real_sol = None):
        """
        ...
        if real_sol:
            ax.scatter(real_sol[0], real_sol[1], c="orange", s=110, marker="X", label="Real solution", zorder=2)

        # Plot our fair solution
        ax.scatter(fair_sol[0], fair_sol[1], c="red", s=220, marker="*", label="Fair solution", zorder=4)
    ...
    def par_npar_plot(self, satpoints, nparpoints, fair_sol, savefile, realsol = None):
        """
        ...
        # If required, plot the point of the real solution's utilities
        if realsol:
            ax.scatter(realsol[0], realsol[2], c="orange", s=110, marker="X", label="Real solution", zorder=2)

        # Plot our fair solution
        ax.scatter(fair_sol[0], fair_sol[2], c="red", s=220, marker="*", label="Fair solution", zorder=4)
    ...
    def gov_npar_plot(self, algpoints, nparpoints, fair_sol, savefile, realsol = None):
        """
        ...
        # If required, plot the point of the real solution's utilities
        if realsol:
            ax.scatter(realsol[1], realsol[2], c="orange", s=110, marker="X", label="Real solution", zorder=2)

        # Plot our fair solution
        ax.scatter(fair_sol[1], fair_sol[2], c="red", s=220, marker="*", label="Fair solution", zorder=4)
